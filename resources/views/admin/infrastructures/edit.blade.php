    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.infrastructures.update", [$infrastructure->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('photos') ? 'is-invalid' : '' }}" type="text" name="photos" id="photos" value="{{ old('photos', $infrastructure->photos) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('days') ? 'is-invalid' : '' }}" name="days[]" id="days" multiple required>
                    @foreach($days as $id => $day)
                        <option value="{{ $id }}" {{ (in_array($id, old('days', [])) || $infrastructure->days->contains($id)) ? 'selected' : '' }}>{{ $day }}</option>
                    @endforeach
                </select>
                @if($errors->has('days'))
